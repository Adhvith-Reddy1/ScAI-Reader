...
def _draw_placeholder_chart(c: canvas.Canvas, x0: float, y0: float, x1: float, y1: float) -> None:
    ...
    human can see whether the box actually lands on "the figure". It also
    gives PdfBackend.get_page_graphics real vector-path objects to enumerate."""
    c.saveState()
...
def build_figure_doc(path: Path) -> None:
    ...
    real pipeline — PDFium text extraction, column clustering, detect_figures,
    AND graphics-based bbox tightening together — which the synthetic-PageText
    unit tests in test_figures.py cannot, since those build
    TextRun/TextColumn/BBox fixtures by hand."""
    c = canvas.Canvas(str(path), pagesize=letter)
